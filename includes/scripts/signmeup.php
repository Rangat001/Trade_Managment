...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
  //                                           Dealer Registration
  $buissness_name = $_POST["buissness_name"];
  ...
  if (empty($buissness_name) || empty($owner_name) || empty($email) || empty($phone) || empty($password) || empty($confirmPassword)) {
  ...
  //                                                                  Dealer Reg.
   $stmt_insert = $conn->prepare("INSERT INTO `dealer` ( `buissness_name`, `owner_name`,`email`, `phone`,`plan`,`created_at`) VALUES (?,?,?,?,?,current_timestamp())");   // 5 pram

   // check insert query ir not
    if ($stmt_insert){
 
     $stmt_insert->bind_param("sssss",$buissness_name,$owner_name,$email,$phone,$plan);
     if ($stmt_insert->execute()) {
       // dealer Registration successful
      $dealer_id = $stmt_insert->insert_id;

      // Now insert into user_master table
      $insertQuery = "INSERT INTO `users` (`dealer_id`, `name`,`email` ,`password`, `role`,`'is_verified'`) VALUES (?,?,?,?, 'ADMIN',0)";
      if($insertQuery){
        $insert->bind_param("isss",$dealer_id,$owner_name,$email,$hashedpassword);
